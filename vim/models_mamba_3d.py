...
from utils_3d.utils_3d import PatchEmbed3D
...
class VisionMamba3D(nn.Module):
    ...
    - Removed num_classes
    - Removed cls_token
    - Changed default values (img_size: 224 -> 512, embed_dim: 192 -> not sure, channels: 3 -> 1)
    """
    def __init__(self, 
                 img_size=224, 
                 patch_size=16, 
                 depth=24, 
                 embed_dim=192, # not sure
                 channels=1, 
                 ssm_cfg=None, 
                 drop_rate=0.,
                 drop_path_rate=0.1,
        ...
        # self.if_cls_token = if_cls_token # class token shouldn't be needed for binary segmentation
        ...
        self.patch_embed = PatchEmbed3D(
            patch_size=patch_size, in_chans=channels, embed_dim=embed_dim) # for 3D
        ...
        # from UNETR/SegMamba
        self.head = UnetOutBlock(spatial_dims=3, in_channels=48, out_channels=13)
