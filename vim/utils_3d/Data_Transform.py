...
class RandomFlip_LR:
    ...
    def __call__(self, img, mask, addition=None):
        prob = random.uniform(0, 1)
        if addition is not None:
            return self._flip(img, prob), self._flip(mask, prob), self._flip(addition, prob)
        else:
...
class Compose:
    ...
    def __call__(self, img, mask, addition=None):
        if addition is None:
            ...
            for t in self.transforms:
                img, mask, addition = t(img, mask, addition)
            return img, mask, addition
